...
resultsTime = zeros(size(maps,1),numberOfrepeats);
...
checkSum = md5(strcat(pwd,'\BotSim.m'))
if(strcmp(checkSum, 'B0C6C4418E11B8D362DA430B938502BE') ==0)
    ...
    for i = 1:size(maps,1)
        for j=1:size(noiseLevel,2)
            ...
            for k = 1:numberOfrepeats
                fprintf('map %0.f\t noiseLevels %0.f \t repeat %0.f',i, j, k);
                it = it+1;
                clf;        %clears figures
                ...
                try
                    resultsTimer=    tic; %starts timer
                    %calls your (hopefully finished) localisation function
                    returnedBot = localise(botSim,maps{i},target);
                    
                    %results calculation
                    %                 resultsTime(i,k) = toc(resultsTimer); %stops timer
                    %                 resultsDis(i,k) =  distance(target, botSim.getBotPos(adminKey));
                    path = returnedBot.getBotPath(adminKey);
                    resTemp(it,1) = toc(resultsTimer); %stops timer
                    resTemp(it,2) = distance(target, botSim.getBotPos(adminKey));
                    pathLength = 0;
                    collided = 0;
                    ...
                    %                 resultsLength(i,it) = pathLength;
                    %                 resultsCollision(i,it) = collided;
                    
                    resTemp(it,3) =  pathLength;
                    resTemp(it,4) =  collided;
                    fprintf('Time: %.3f, Distance: %.3f, Length: %.3f, Collision: %.0f\n',resTemp(it,1),resTemp(it,2),resTemp(it,3),resTemp(it,4));
                    
                    %                 fprintf('Time: %.3f, Distance: %.3f, Length: %.3f, Collision: %.0f\n',resultsTime(i,k), resultsDis(i,k),pathLength,collided);
                catch err
                    %                 resultsTime(i,k) = 0;
                    %                 resultsDis(i,k) = 0;
                    %                 resultsLength(i,k) = 0;
                    %                 resultsCollision(i,k) = -1;
                    resTemp(it,:) = [-1,-1,-1,-1];
                    err.identifier
                end
            end
    ...
    %     compiledResults = [resultsTime resultsDis resultsLength resultsCollision]
    ResultsTable = table(resultsTime, resultsDis, resultsLength, resultsCollision,'RowNames',{'Map1';'Map2'})
